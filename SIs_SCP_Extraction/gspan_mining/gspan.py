...
flis2=collections.defaultdict(list)
...
class gSpan(object):
    ...
    def time_stats(self):
        ...
        fk.write('No of subgraphs')
        fk.write(" ")
        global nsg
        fk.write(str(nsg[si_id]))
        fk.write(" ")
    ...
    def _report(self, projected):
        ...
        # neighbor set
        sorted_vertices=collections.defaultdict()

        for vid in g.vertices:
            vertex_set[vid]=g.vertices[vid].vlb

        neighbor_degree=collections.defaultdict(list)
        sorted_vertices=sorted(vertex_set.items(), key=lambda x: x[1], reverse=True)
        ...
        # deg=degree.most_common()
        degree_vwd=collections.Counter()
        for i in degree:
            degree_vwd[i]=0
            # print("degree",i)
        si_degree_vwd=collections.Counter()
        for i in vert:
            si_degree_vwd[i]=0            
        for frm in g.vertices:
            edges=g.vertices[frm].edges
            for to in edges:
                if frm<to:
                    neighbor_degree[frm].append(degree[to])
                    neighbor_degree[to].append(degree[frm])
        list_of_gids=collections.defaultdict(list)
        list_of_gids=set([p.gid for p in projected])
        
        # print("nd",neighbor_degree)
        ...
        if(no_of_edges==noe ):
            for i in range(len(degree)):
                for j in range(len(vert)):
                    ...
                    if(degree[i]==vert[j] and  sorted(neighbor_degree[i])==sorted(ne[j])  and degree_vwd[i]==0 and si_degree_vwd[j]==0):
                        # print(sorted(neighbor_degree[i]),sorted(ne[j]))
                        degree_vwd[i]=1
                        si_degree_vwd[j]=1
                        flag += 1
        

        # print("vwd",degree_vwd)
        # print("vbd",si_degree_vwd)
        # print("flag",flag)
        if(flag==len(vert)):
            #print("gids",list_of_gids)
            #print("yes si",si_id)
            #print("nd",neighbor_degree)
            #print("degree",degree)
            global arr
            ...
            for i in list_of_gids:
                flis[arr].append(i)
            # print(flis)
            global flis1 #is id mapped to id of subgraph
            flis1[arr].append(si_id)
            global flis2 # id and subgraph
            flis2[si_id].append(arr)
        ...
        # print("sample",degree[1],si_degree[1])  
        if(flag==len(vert) ):
